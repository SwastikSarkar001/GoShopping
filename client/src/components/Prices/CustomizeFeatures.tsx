...
export default function CustomizeFeatures({ features, isLoadingFeatures, tiers, isLoadingTiers, featuresDispatch }: CustomizeFeaturesProps) {
...
function AddFeature({ features, isLoadingFeatures, tiers, isLoadingTiers, setFunc }: AddFeatureProps) {
  ...
  const tierOpt = selectedFeature !== null ? 
    ...
    null
  if (isLoadingFeatures || isLoadingTiers) 
  return (
    <div className="py-4 px-8 rounded-4xl flex gap-4 items-center cursor-not-allowed bg-gray-300/20 text-gray-500 w-full">
      <div>
  ...
  else if (features.filter(feature => feature.tier === 0).length === 0) {
    return null
  }
  else
  return (
            ...
            </Select>
            {
              ...
              <>
                ...
                <div className="grid grid-cols-2 mt-2 items-center justify-between *:even:text-right">
                  ...
                  <div className="font-bold">{value} ({tiers.filter(tier => tier.tier === value)[0].tierName})</div>
...
function SelectedFeatures({ features, isLoadingFeatures, tiers, isLoadingTiers, deselectFunc, setFunc }: SelectedFeaturesProps) {
  const [selectedFeature, setSelectedFeature] = useState<{value: string, label: string} | null>(null)
  const [value, setValue] = useState<number>(1); // Initial value of the slider
  const tierOpt = selectedFeature !== null ? 
    tierAndPriceCalculator(
          ...
          feature => 
            ...
            <div key={feature.featureID} className="border-2 border-gray-400/30 rounded-4xl py-4 px-8 gap-8 flex justify-between items-center">
              ...
              <div className="flex items-center gap-4 text-sm mr-8">
                <div className="text-gray-400">
                  Maximum users: {tierAndPriceCalculator(tiers, feature.tier, feature.price_per_user_per_month).users}
                </div>
              </div>
              <div id={`popover ${feature.featureID}`} className="group relative">
                <button
